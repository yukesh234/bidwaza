...
import { getListing, updateStock } from '../../services/sellerservices.js'
import toast from 'react-hot-toast'

function Listing({ onDelete, onStatusChange }) {
    ...
    try {
      ...
      if (result.success) {
        ...
        const transformedListings = result.products.map(product => ({
          ...
          registrationEnd: product.registrationEnd
        }))
  ...
  const handleModalClose = () => {
    ...
    setEditingListing(null)
    // Optionally refetch listings after modal closes
    // fetchListings(currentPage, searchTerm, filterStatus)
  }
  ...
  const getBidhistory = async (itemId) => {
    try {
